...
/* wolfCrypt's stock benchmark, the same program the A55 container runs. It
...
/* Append bytes to wolfBoot's shared-memory console ring. wolfBoot is done
 * writing by the time this payload runs, so there is no writer to race; the
 * A55 demo only ever reads. Data is cleaned before the header so a reader
 * never sees wr ahead of the bytes it covers.
 *
 * Only the bytes just written are cleaned. Flushing the whole ring instead
 * would move 64 KiB per call, which is wasteful for a heartbeat line and
 * unusable once this is the console and each character arrives on its own. */
static void ring_write(const char *buf, size_t len)
{
    struct console_hdr *hdr = (struct console_hdr *)CONSOLE_BASE;
    uint8_t *data = (uint8_t *)(CONSOLE_BASE + CONSOLE_HDR_SIZE);
    uint32_t size = hdr->size;
    uint32_t wr, start, first;
    size_t i;

    if (hdr->magic != CONSOLE_MAGIC || size == 0U ||
        size > (0x10000U - CONSOLE_HDR_SIZE) || len == 0U) {
        return;
    }
    wr = hdr->wr;
    start = wr % size;
    for (i = 0; i < len; i++) {
        data[(wr + i) % size] = (uint8_t)buf[i];
    }
    /* One contiguous run, plus the wrapped remainder when it crosses the end. */
    first = size - start;
    if ((uint32_t)len <= first) {
        sys_cache_data_flush_range((void *)&data[start], len);
    }
    else {
        sys_cache_data_flush_range((void *)&data[start], first);
        sys_cache_data_flush_range((void *)data, (uint32_t)len - first);
    }
    hdr->wr = wr + (uint32_t)len;
    sys_cache_data_flush_range((void *)hdr, CONSOLE_HDR_SIZE);
}

static void ring_puts(const char *str)
{
    ring_write(str, strlen(str));
}

/* Send everything Zephyr prints - the boot banner, driver init, LOG output and
 * any printk in this app - to the same ring wolfBoot used, so one reader on the
 * A55 sees the whole story of this core rather than just the lines this file
 * writes by hand. Zephyr's own RAM console is a separate buffer nobody reads.
 *
 * Installed from PRE_KERNEL_1 so it is in place before the kernel prints its
 * banner; ring_puts() only touches DDR and the cache, both of which are usable
 * that early. */
static int ring_char_out(int c)
{
    char ch = (char)c;
...
static int ring_console_init(void)
{
    __printk_hook_install(ring_char_out);
    return 0;
}

SYS_INIT(ring_console_init, PRE_KERNEL_1, 0);
...
int main(void)
{
    ...
    /* Say what this core actually is and what it was handed, rather than
     * leaving the demo to infer it. The vring and buffer addresses are the
     * contract wolfBoot's resource table declared to Linux, so printing them
     * here is what makes a mismatch visible instead of a silent hang. */
    ring_puts("\r\n");
    ...
    printk("payload verified by wolfBoot (ML-DSA-87) and entered at %p\n",
           (void *)main);
    printk("rpmsg: vring0 0x%08lx vring1 0x%08lx, %u descriptors each\n",
           (unsigned long)VRING0_DA, (unsigned long)VRING1_DA,
           (unsigned)VRING_NUM);
    /* Microseconds: the whole Zephyr startup is well under a millisecond, so
     * milliseconds would just print zero. */
    printk("reached main %llu us after Zephyr entry\n",
           (unsigned long long)k_cyc_to_us_floor64(k_cycle_get_32()));
    ...
    ring_puts("\r\nZephyr on Cortex-M7: verified payload running, "
              "RPMsg tty up\r\n");

    /* Run wolfCrypt's own benchmark once, so this core reports the same
     * algorithms in the same units as the A55 cluster does. It prints through
     * printk, which this app has pointed at the shared ring, so it lands in
     * the same stream as wolfBoot's verified-boot output above. */
    {
    ...
    while (1) {
        ...
        /* Proof of life on the shared console every ~10 s (the loop ticks at
         * ~20 Hz on the 50 ms kick timeout), so the demo pane shows the
         * verified payload doing work rather than freezing after boot. */
        if ((beat % 200U) == 0U) {
            char hb[64];
            snprintk(hb, sizeof(hb),
                     "M7 heartbeat %u: verified payload alive\r\n",
                     beat / 200U);
            ring_puts(hb);
        }
